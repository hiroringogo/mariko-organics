export const t = {
  // Tab bar
  home: { ja: "ホーム", en: "Home" },
  book: { ja: "予約", en: "Book" },
  shop: { ja: "ショップ", en: "Shop" },
  myPage: { ja: "マイページ", en: "My Page" },

  // Home page
  heroTag: { ja: "グルテンフリーワークショップ", en: "Gluten-Free Workshop" },
  heroSub: { ja: "〜心と体にやさしいお米の新しい調理法〜", en: "~A new way of cooking with rice, gentle on mind and body~" },
  heroTitle: { ja: "3月：生米パン・ド・ロー", en: "March: Raw Rice Pão de Ló" },
  heroDesc: { ja: "ポルトガル風リゾットランチ付き", en: "With Portuguese-style risotto lunch" },
  classSchedule: { ja: "レッスン日程", en: "Class Schedule" },
  viewMore: { ja: "他の日程も見る →", en: "View more dates →" },
  announcements: { ja: "お知らせ", en: "Announcements" },
  memberPromo: { ja: "プレミアメンバー受付中！", en: "Premium Member registration open!" },
  memberPromoSub: { ja: "年会費で特典いっぱい。詳しくはこちら →", en: "Annual fee with great perks. Learn more →" },

  // Lesson card
  lessonTitle: { ja: "生米パン・ド・ロー", en: "Raw Rice Pão de Ló" },

  // Lesson list
  lessonList: { ja: "レッスン一覧", en: "All Lessons" },
  noLessons: { ja: "現在予約可能なレッスンはありません", en: "No lessons currently available" },

  // Lesson detail
  lessonDetail: { ja: "レッスン詳細", en: "Lesson Details" },
  lessonContentsTitle: { ja: "レッスン内容", en: "What You'll Make" },
  content1Title: { ja: "生米パン・ド・ロー作り", en: "Raw Rice Pão de Ló" },
  content1Sub: { ja: "ポルトガルのふわふわスポンジケーキを生米から！", en: "Fluffy Portuguese sponge cake from raw rice!" },
  content2Title: { ja: "ポルトガル風リゾットランチ", en: "Portuguese-style Risotto Lunch" },
  content2Sub: { ja: "テーマに合わせた特別メニュー付き", en: "Special themed menu included" },
  approxHours: { ja: "（約3時間）", en: "(approx. 3 hours)" },
  locationDetail: { ja: "Orange County, CA（詳細は予約後にご案内）", en: "Orange County, CA (details shared after booking)" },
  perLesson: { ja: "/ レッスン", en: "/ lesson" },
  selectDate: { ja: "日程を選択", en: "Select a Date" },
  full: { ja: "満席", en: "Full" },

  // Booking form
  customerInfo: { ja: "お客様情報", en: "Your Information" },
  nameLabel: { ja: "お名前", en: "Full Name" },
  namePlaceholder: { ja: "例：田中 花子", en: "e.g. Jane Smith" },
  emailLabel: { ja: "メールアドレス", en: "Email Address" },
  emailPlaceholder: { ja: "example@email.com", en: "example@email.com" },
  participantCount: { ja: "参加人数", en: "Number of Guests" },
  companionNames: { ja: "同伴者のお名前", en: "Companion Names" },
  notesLabel: { ja: "備考（アレルギーなど）", en: "Notes (allergies, etc.)" },
  notesPlaceholder: { ja: "ご自由にご記入ください", en: "Feel free to write anything here" },
  referredByLabel: { ja: "紹介者のお名前", en: "Referred by" },
  referredByPlaceholder: { ja: "どなたのご紹介ですか？", en: "Who referred you?" },
  referredBy: { ja: "紹介者", en: "Referred by" },
  noRegistration: { ja: "※ メンバー登録不要で予約できます", en: "* No account registration required to book" },

  // Booking policy
  cancellationTitle: { ja: "ご予約について", en: "About Your Booking" },
  cancellationBullet0: {
    ja: "予約確定後、詳細についてメールをお送りします",
    en: "After your booking is confirmed, we will send you the details via email.",
  },
  cancellationBullet1: {
    ja: "やむを得ずにお越しいただけない場合は、同月または翌月のレッスンのお振替が可能です",
    en: "If you are unable to attend, you may reschedule to a lesson within the same or next month.",
  },

  // Booking confirm page
  confirmBooking: { ja: "予約確認", en: "Confirm Booking" },
  representative: { ja: "代表者", en: "Group Leader" },
  emailConfirm: { ja: "メールアドレス", en: "Email" },
  guests: { ja: "参加人数", en: "Guests" },
  companions: { ja: "同伴者", en: "Companions" },
  total: { ja: "合計", en: "Total" },
  proceedToPayment: { ja: "予約を確定する", en: "Confirm Booking" },

  // Booking complete page
  bookingConfirmed: { ja: "予約が確定しました！", en: "Booking Confirmed!" },
  confirmEmailSent: {
    ja: "確認メールをお送りしました。\n当日の詳細はメールをご確認ください。",
    en: "A confirmation email has been sent.\nPlease check your inbox.",
  },
  amountPaid: { ja: "レッスン料金", en: "Lesson Fee" },
  backToHome: { ja: "ホームに戻る", en: "Back to Home" },
  viewInMyPage: { ja: "マイページで確認", en: "View in My Page" },

  // My Page
  myPageTitle: { ja: "マイページ", en: "My Page" },
  emailLookupPrompt: { ja: "予約時に使用したメールアドレスで\n予約状況を確認できます", en: "Check your booking status with the\nemail used for your reservation" },
  emailInputPlaceholder: { ja: "メールアドレスを入力", en: "Enter your email" },
  checkBookings: { ja: "予約を確認する", en: "Check Bookings" },
  noBookingsFound: { ja: "このメールアドレスでの予約は見つかりませんでした", en: "No bookings found for this email" },
  upcomingLessons: { ja: "予約中のレッスン", en: "Upcoming Lessons" },
  noUpcoming: { ja: "予約中のレッスンはありません", en: "No upcoming lessons" },
  findLessons: { ja: "レッスンを探す →", en: "Find lessons →" },
  details: { ja: "詳細", en: "Details" },
  cancel: { ja: "キャンセル", en: "Cancel" },
  lessonHistory: { ja: "レッスン履歴", en: "Lesson History" },
  cancelled: { ja: "キャンセル済み", en: "Cancelled" },
  edit: { ja: "変更", en: "Edit" },
  editBooking: { ja: "予約を変更", en: "Edit Booking" },
  changeDate: { ja: "レッスン日の変更", en: "Change Lesson Date" },
  changeParticipants: { ja: "参加人数の変更", en: "Change Number of Guests" },
  saveChanges: { ja: "変更を保存する", en: "Save Changes" },
  bookingUpdated: { ja: "予約を変更しました", en: "Booking updated" },

  // Membership
  membership: { ja: "メンバーシップ", en: "Membership" },
  memberTitle: { ja: "Mariko Organics\nメンバーシップ", en: "Mariko Organics\nMembership" },
  memberFee: { ja: "年会費 $40 で特典いっぱい", en: "Full of benefits with $40 annual fee" },
  memberBenefits: { ja: "メンバー特典", en: "Member Benefits" },
  benefit1Title: { ja: "プレミアメンバー限定レッスン", en: "Premium Member Exclusive Lessons" },
  benefit1Desc: { ja: "メンバー限定のレッスンに参加可能", en: "Access lessons exclusive to Premium Members" },
  benefit2Title: { ja: "メンバー限定クラス", en: "Members-Only Classes" },
  benefit2Desc: { ja: "特別なテーマのプライベートレッスン", en: "Access exclusive classes available only to members" },
  benefit3Title: { ja: "ショップの限定販売", en: "Exclusive Products" },
  benefit3Desc: { ja: "米粉やオリジナル商品の優先購入", en: "Get early access to ingredients and limited-edition products" },
  benefit4Title: { ja: "過去レシピのオンライン閲覧", en: "Online Recipe Archive" },
  benefit4Desc: { ja: "これまでのレッスンレシピをいつでも確認", en: "Browse past class recipes online anytime" },
  memberCTA: { ja: "メンバーシップに申し込む", en: "Become a Member" },
  memberFeeNote: { ja: "年会費 $40（税込）", en: "Annual fee $40 (tax included)" },
} as const;

// Date/time formatting helpers
export const dayNames = {
  ja: ["日", "月", "火", "水", "木", "金", "土"],
  en: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
} as const;

const monthNamesEn = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

export function formatMonth(m: number, lang: "ja" | "en") {
  return lang === "ja" ? `${m}月` : monthNamesEn[m - 1];
}

export function formatDateFull(dateStr: string, lang: "ja" | "en") {
  const d = new Date(dateStr + "T00:00:00");
  const m = d.getMonth() + 1;
  const day = d.getDate();
  const dow = dayNames[lang][d.getDay()];
  if (lang === "ja") return `${m}月${day}日（${dow}）`;
  return `${dow}, ${monthNamesEn[m - 1]} ${day}`;
}
